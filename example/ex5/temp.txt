...
def home():
    ...
    location = getlocation(request.cookies.get("location"))
    if location == False:
...
class TimesException(Exception):
    def __init__(self):
        super(TimesException, self).__init__("Call func too many times!")
...
def loads(strs):
    ...
    unpkler.dispatch[pickle.REDUCE] = _hook_call(unpkler.dispatch[pickle.REDUCE])
    return unpkler.load()
